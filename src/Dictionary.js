export const getMessages = (variant, variable, justMessage) => {
  let dic = {
    noEntity: `Looks like there no ${variable} on this store/category/subcategory.`,
    noStore: `Looks like there no ${variable} made yet, if you are a seller and want to create a store please on the button below`,
    noCategoryGeneral:
      'Category helps in organizing products which helps customer to find the product easily. No categories have been added yet, click the button below to create one.',
    noProduct:
      'No products have been added yet, click the button below to add a product',
    noProductCustomer:
      'Sorry! No products/services found, please check after some time',
    storeDeleted:
      'Looks like store is deleted or no categories have been added yet to the store, please go back to home page and checkout other stores.',
    'Order Placed': 'You order has been place successfully',
    Packed: 'Your order is packed.',
    Dispatched: 'Your order is dispatched',
    'Out For Delivery': 'Your order is out for delivery',
    Delivered: 'Your product has been delivered.',
  }
  return justMessage ? dic[variant] : <div>{dic[variant]}</div>
}
...
export const getSingularFromPlural = v => {
  if (v && ['colors', 'color'].includes(String(v).toLowerCase())) {
    return 'color'
  } else if (v && ['sizes', 'size'].includes(String(v).toLowerCase())) {
    return 'size'
  } else if (v && ['patterns', 'pattern'].includes(String(v).toLowerCase())) {
    return 'pattern'
  } else if (v && ['styles', 'style'].includes(String(v).toLowerCase())) {
    return 'style'
  } else return v
}
